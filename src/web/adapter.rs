...
use crate::{Error, Result, common::adapter_manager::AdapterManager, web::tauri};
    ...
    async fn start_scan(&self, filter: ScanFilter) -> Result<()> {
        if is_tauri() {
            ...
            spawn_local(async move {
                ...
                .await
                .expect("Failed to scan using Tauri");
            ...
            spawn_local(async move {
                ...
                let devices = vec![BluetoothDevice::from(
                    JsFuture::from(get_bluetooth_api().request_device(&options))
                        .await
                        .map_err(|x| {
                ...
                for device in devices {
                    ...
                    if let Some(mut entry) = manager_clone.peripheral_mut(&uuid.into()) {
                        ...
                        let peripheral =
                            Peripheral::new(Arc::downgrade(&manager_clone), uuid, id, name, vec![]);
                        peripheral.update_properties().await;
                        manager_clone.add_peripheral(peripheral);
                        manager_clone.emit(CentralEvent::DeviceDiscovered(uuid.into()));
                    }
